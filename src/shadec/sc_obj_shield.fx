...
float4 data1; // y= softness, z=tex movement, w = spark texture offset
...
sampler sparkSampler = sampler_state
{
	Texture = <entSkin3>;
	MinFilter = LINEAR;
	MagFilter = LINEAR;
					  ...
					  ):COLOR0
{
   ...
   float texDepth = UnpackDepth(tex2D(normalsAndDepthSampler, inProjectedTex.xy).zw);
	...
	pixelNormal = lerp(tex2D(normalmap2Sampler,inTex+(vecTime.w*0.001*data1.z)),pixelNormal,0.5);
   pixelNormal.yz=pixelNormal.zy;
   pixelNormal.xyz=(pixelNormal.xyz-0.5f)*2;

   float4 refraction=tex2D(sceneSampler,(inProjectedTex.xy)-refractStrength*pixelNormal.xz);
   float4 spark = tex2D(sparkSampler,(inTex/float2(8,1))+float2(data1.w,0));
	float4 sparkR = tex2D(sparkSampler,((inTex/float2(8,1))+float2(data1.w,0))-refractStrength*pixelNormal.xz);
	
	spark.rgb += spark.rgb*data3.xyz;
	spark.rgb = pow(spark.rgb,1.7);
	sparkR.rgb += sparkR.rgb*data3.xyz;
	sparkR.rgb = pow(sparkR.rgb,1.7);
	
	spark.rgb *= spark.a;
	sparkR.rgb *= sparkR.a*4;
	
	float4 color = 1;
	color = refraction;
	//color.gb += refraction.gb;
	
	#ifdef USECOLOR
		color.rgb *= tex2D(colorSampler,inTex).xyz;
	#endif
	
	color.a = tex2D(normalmapSampler,inTex).a*fAlpha*difference;
	color.rgb += (spark.rgb+sparkR.rgb)*4;
	
	//color.rgba = 1;
	//color.rgb = tex2D(sceneSampler, inProjectedTex.xy).xyz;
	
	//color.rgb = spark + sparkR;// + sparkR;
