...
texture texAtlas;
sampler3D atlasSampler = sampler_state
{
	Texture = <texAtlas>;
   AddressU = WRAP; 
	AddressV = WRAP;
...
PixelToFrame mainPS(vsOut In)
{
	...
	/*
	half2 noise = tex2D(entSkin2Sampler, In.Tex*8).xy*2-1;
	noise += (tex2D(entSkin2Sampler, In.Tex*512).xy*2-1);
	half2 tempNoise = noise + (tex2D(entSkin2Sampler, In.Tex*16).xy*2-1)*0.5;
	float terrainMask = tex2D(entSkin1Sampler, In.Tex + tempNoise*0.01).xyz*32*8;
	tempNoise = noise + (tex2D(entSkin2Sampler, In.Tex*32).xy*2-1)*0.5;
	float terrainMask2 = tex2D(entSkin1Sampler, In.Tex + tempNoise*0.01).xyz*32*8;
	tempNoise = noise + (tex2D(entSkin2Sampler, In.Tex*64).xy*2-1)*0.5;
	float terrainMask3 = tex2D(entSkin1Sampler, In.Tex + tempNoise*0.01).xyz*32*8;
	tempNoise = noise + (tex2D(entSkin2Sampler, In.Tex*128).xy*2-1)*0.5;
	float terrainMask4 = tex2D(entSkin1Sampler, In.Tex + tempNoise*0.01).xyz*32*8;
	
	half3 color1 = tex3D(atlasSampler, half4(In.Tex.xy*10,((terrainMask+0.5f)/8.f),0)).xyz;
	half3 color2 = tex3D(atlasSampler, half4(In.Tex.xy*10,((terrainMask2+0.5f)/8.f),0)).xyz;
	half3 color3 = tex3D(atlasSampler, half4(In.Tex.xy*10,((terrainMask3+0.5f)/8.f),0)).xyz;
	half3 color4 = tex3D(atlasSampler, half4(In.Tex.xy*10,((terrainMask4+0.5f)/8.f),0)).xyz;
	
	
	PSOut.albedoAndEmissiveMask.xyz = (color1+color2+color3+color4)/4;
	*/
	
	half2 noise = tex2D(entSkin2Sampler, In.Tex*2).xy*2-1;
	float terrainMask = tex2D(entSkin1Sampler, In.Tex).xyz*32*8;
	float terrainMaskNoise = tex2D(entSkin1Sampler, In.Tex + (noise)*0.01).xyz*32*8;
	//PSOut.albedoAndEmissiveMask.xyz = tex3D(atlasSampler, half4(In.Tex.xy*20,((terrainMask+0.5f)/8.f),0)).xyz * tex3D(atlasSampler, half4(In.Tex.xy,((terrainMask+0.5f)/8.f),0)).a;
	
	half3 color = tex3D(atlasSampler, half4(In.Tex.xy*10,((terrainMask+0.5f)/8.f),0)).xyz;
	half3 colorNoise = tex3D(atlasSampler, half4(In.Tex.xy*10,((terrainMaskNoise+0.5f)/8.f),0)).xyz;
	
	PSOut.albedoAndEmissiveMask.xyz = lerp( colorNoise, 0, 1-saturate(noise.x*0.5+0.5) );
	//PSOut.albedoAndEmissiveMask.xyz /= 2;
	//PSOut.albedoAndEmissiveMask.xyz = pow(In.Tex.x - (In.Tex.x+noise.x),0.5);
