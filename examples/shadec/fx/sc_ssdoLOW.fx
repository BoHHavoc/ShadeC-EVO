...
float4 mainPS(float2 inTex:TEXCOORD0):COLOR0
{
	...
	half ao = 0;
	half aoCurrent = compareDepths(gBuffer.w,depth2, (aoRadius+3)*2, normal);
	ao += aoCurrent;
	half3 color = tex2D( albedoAndEmissiveSampler, samp_UV ).xyz * aoCurrent;// * UnpackLighting(tex2D(lightingSampler, samp_UV)).xyz;
	...
	aoCurrent = compareDepths(gBuffer.w,depth2, (aoRadius+3)*2, normal);
	ao += aoCurrent;
	color += tex2D( albedoAndEmissiveSampler, samp_UV ).xyz * aoCurrent;// * UnpackLighting(tex2D(lightingSampler, samp_UV)).xyz;
	...
	return half4(color,ao);//half4(1-saturate(ao.xxx),1);
}
