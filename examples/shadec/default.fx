	...
	//section: scLights - contains all needed stuff to compute deferred lighting  //
		...
		*/
		...
		technique t1
		{
				...
				pass pSunBackdrop
				{
					PixelShader = compile ps_3_0 sunBackdrop();
	...
	//section: scObject - Shade-C object ubershader //
		...
		vsOut mainVS(vsIn In)
		{
			...
			//WIND ANIMATION
				...
				float force_x = vecSkill41.x*(0.1/50); 
				float force_y = vecSkill41.y*(0.1/50);
				float speed = sin((vecTime.w+0.2*(P.x+P.y+P.z)) * vecSkill41.z*(0.05/50));
				
				if (In.Pos.y > vecSkill41.w) // move only upper part of tree
				{
			...
			Out.Fog =clamp((distance(mul(In.Pos, matWorld), vecViewPos) - vecFog.x) * (vecFog.z),0,1);
...
//			#ifndef TARGET_VS
//				#define TARGET_VS vs_3_0
//			#endif
//			#ifndef TARGET_PS
//				#define TARGET_PS ps_3_0
//			#endif
	...
	//section: scLightShadowmap - shadowmap calculation //
		...
		technique LightShadowmap_lm
		{
			pass p0
			{
				...
				zwriteenable = true;
				vertexshader = compile vs_3_0 mainVS();
				pixelshader = compile ps_3_0 mainPS_lm();
			}
		}
	#endif
