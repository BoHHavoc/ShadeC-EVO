...
texture sc_materials_fogMap_bmap; //fog
...
{
	Texture = <sc_materials_fogMap_bmap>;
...
float4 mainPS(psIn In):COLOR
{
	...
	half4 fogMap = tex2D(fogSampler, In.Tex);
...
//	output.xyz += albedoAndEmissiveMask.xyz * albedoAndEmissiveMask.w;
	...
	//output.xyz = diffuseAndSpecular.xyz + diffuseAndSpecular.w * diffuseAndSpecular.xyz;
	float3 fogColor = pow(vecFogColor.rgb, 2.2);
	output.xyz = lerp(fogColor,output.xyz,1-clamp(fogMap.w,0.f,1.f));
//	output.x=albedoAndEmissiveMask.w;
	output.w = 1;
