...
float4 mainPS(vsOut In):COLOR0
{
   ...
   half3 Vn = normalize(vecViewDir.xyz - posVS); //less arithmetic instructions
   half3 Hn = normalize(Vn + normalize(Ln));
   ...
   half2 light = lit(dot(Ln,normal), dot(Hn, normal),15).yz;
	...
	light.x = light.x * att;
   color.rgb = light.x * vecSkill5.xyz * texCUBE(projSampler, mul(mul(Ln, matViewInv),matMtl));//vecSkill5.xyz;
   //color.rgb /= 2;
   //color.a = light.y * length(color.rgb);
   color.a = light.x * light.y;
